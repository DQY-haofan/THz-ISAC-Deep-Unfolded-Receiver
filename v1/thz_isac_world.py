"""
thz_isac_world.py (Wideband Delay Model - Expert Patched v7.1)
...
Expert Patches Applied:
- Patch-0: Fixed compute_bcrlb_diag() to use consistent SINR_eff definition
           (harmonic mean + chi scaling, avoid double counting)
"""
...
# =============================================================================
# EXPERT PATCH-0: Fixed compute_bcrlb_diag()
# =============================================================================
#
...
#
# Issues:
#   1. Inconsistent with simulate_batch() which uses harmonic mean for SINR_eff
#   2. gamma_eff may already include quantization distortion power, causing double counting
#
# Fixed implementation:
#   1. Use harmonic mean: SINR_eff = 1/(1/SNR + 1/Gamma_eff)
#   2. For 1-bit, scale by chi(rho) instead of multiplying quantization_loss
# =============================================================================

def compute_bcrlb_diag(config: SimConfig, snr_linear: float, gamma_eff: float) -> np.ndarray:
    """
    Compute diagonal elements of Bayesian CRLB for [τ, v, a].

    Expert Patch-0: Fixed to use consistent SINR_eff definition.

    Key changes:
    1. sinr_eff uses harmonic mean of thermal SNR and hardware Gamma_eff
    2. 1-bit quantization enters via chi(sinr_eff) (information-retention factor)
       rather than multiplying ad-hoc 'quantization_loss' again (avoid double counting)
    ...
    Returns:
        bcrlb: Array of [BCRLB_tau, BCRLB_v, BCRLB_a]
    """
    N = config.N
    B = config.fs
    T = N / config.fs
    ...
    # 1) Consistent SINR_eff via harmonic mean (same as simulate_batch)
    if gm is not None:
        sinr_eff = gm.compute_sinr_eff(snr_linear, gamma_eff)
    else:
        sinr_eff = 1.0 / ((1.0 / snr_linear) + (1.0 / max(gamma_eff, 1e-12)))

    # 2) Quantization information factor (only needed for 1-bit bound guidance)
    #    For few-bit ADC, chi ≈ 1 and bound is mainly driven by sinr_eff
    if config.enable_quantization and config.adc_bits == 1:
        if gm is not None:
            chi = gm.chi_from_rho(sinr_eff)
        else:
            kappa = 1.0 - 2.0 / np.pi
        ...
        # Few-bit: keep bound driven mainly by sinr_eff (already reflects distortion power)
        chi = 1.0

    snr_for_bound = max(sinr_eff * chi, 1e-6)

    # Same formulas, but now with consistent snr_for_bound
    bcrlb_tau = 1.0 / (8 * np.pi**2 * B**2 * N * snr_for_bound)
    bcrlb_v = (c / fc)**2 * 3.0 / (np.pi**2 * T**2 * N * snr_for_bound)
    bcrlb_a = (c / fc)**2 * 180.0 / (np.pi**2 * T**4 * N * snr_for_bound)
...
if __name__ == "__main__":
    print("=" * 70)
    print("thz_isac_world.py (Expert Patched v7.1) - Self-Test")
    print("=" * 70)
    ...
    print(f"  sinr_eff: {data['meta']['sinr_eff']:.4f}")
    print(f"  chi: {data['meta']['chi']:.4f}")

    # Test BCRLB consistency
    print("\n[Patch-0 Verification] BCRLB Consistency Check:")
    snr_linear = data['meta']['snr_linear']
    gamma_eff = data['meta']['gamma_eff']
    bcrlb = compute_bcrlb_diag(cfg, snr_linear, gamma_eff)
    bcrlb_tau_samples = np.sqrt(bcrlb[0]) * cfg.fs
    print(f"  BCRLB_tau: {bcrlb[0]:.4e} s²")
    print(f"  sqrt(BCRLB_tau) in samples: {bcrlb_tau_samples:.4f}")
    print(f"  Uses harmonic SINR_eff: ✓")
    print(f"  Uses chi scaling for 1-bit: ✓")
