...
def train_one_stage(
    ...
    for step in pbar:
        ...
        with torch.no_grad():
            ...
            accept_rate = 1.0
            resid_improve = 0.0
            # v7 Architecture: theta_info is in first layer (TauEstimator runs before VAMP)
            if outputs['layers'] and 'theta_info' in outputs['layers'][0]:
                theta_info = outputs['layers'][0]['theta_info']
                # Handle both old and new theta_info formats
                if 'final_improvement' in theta_info:
                    # New TauEstimator format
                    accept_rate = 1.0
                    resid_improve = theta_info.get('final_improvement', 0.0)
                else:
